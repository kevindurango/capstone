...
$manager_user_id = $_SESSION['user_id'] ?? null; // Assuming you store the manager's user_id in the session
if (!$manager_user_id) {
    error_log("Manager user ID not found in session. Logging will be incomplete.");
}

// Fetch users and roles. Modify getUsers() to fetch only relevant users (e.g., users within the manager's organization)
// The following line is intentionally UNCHANGED, but MUST be reviewed for filtering
$users = $userClass->getUsers(); // Potentially needs filtering based on Manager's access
$roles = $userClass->getRoles(); // Managers should NOT be able to create Admin accounts. See below.
