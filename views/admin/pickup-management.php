...
// Fetch Orders with Pickup Details and Pagination
if (!empty($search)) {
    $query = "SELECT o.order_id, u.username AS consumer_name, o.order_status, o.order_date, o.pickup_details
              ...
              WHERE (u.username LIKE :search OR o.order_status LIKE :search OR o.pickup_details LIKE :search)
              AND o.pickup_details IS NOT NULL
    ...
    $countQuery = "SELECT COUNT(*) AS total 
                   FROM orders 
                   WHERE pickup_details IS NOT NULL";
}
                            ...
                            <?php if (count($orders) > 0): ?>
                                <?php foreach ($orders as $order): ?>
                                    <tr>
                                        ...
                                        <td><?= htmlspecialchars($order['pickup_details']) ?></td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php else: ?>
                                <tr>
                                    <td colspan="5" class="text-center">No pickup details found.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
